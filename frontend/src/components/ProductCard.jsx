...
export default function ProductCard() {
  ...
  const deleteProduct = async() =>{
    try{
      await axios.delete(`${BASE_URL}/api/products/${id}`, id);
      navigate("/");
    } catch(err){
      message.error(err.message);
    }
    ...
    <div className="flex place-items-center justify-center items-center justify-items-center mt-10">
      {product && (
        <div  className="flex flex-row items-center place-items-center">
          <img src={product.imageUrl} className="h-[400px]"></img>
          <div className="w-1/2 h-full place-items-center ml-10">
            ...
            {product && product.available && (
              <div className="inline-flex items-center justify-center whitespace-nowrap rounded-lg text-sm font-medium transition-all bg-blue-800 text-gray-200 shadow hover:bg-blue-900 h-10 w-44">
                Add to Cart
              </div>
            )}
            {product && !product.available && (
